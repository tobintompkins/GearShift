...
import { Vehicle, Job, Employee, InspectionItemStatus } from '@/lib/types';
...
function NewInspectionForm() {
  ...
  useEffect(() => {
    ...
    if (formData.vehicleId) {
      ...
      fetchVehicleMileage(formData.vehicleId);
    }
  }, []);
  ...
  const fetchVehicleMileage = async (vehicleId: string) => {
    try {
      const response = await fetch(`/api/vehicles/${vehicleId}`);
      if (response.ok) {
        const data = await response.json();
        if (data.mileage && !formData.mileage) {
          setFormData((prev) => ({ ...prev, mileage: data.mileage.toString() }));
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create inspection checklist');
      }
      ...
      <main className="container mx-auto px-4 py-4 md:py-8 max-w-6xl">
        <div className="mb-4 md:mb-6">
          ...
          <p className="text-sm md:text-base text-gray-600">
            Professional digital inspection form
          </p>
        </div>
